...
As of 13 September 2026, 53 bundle tests pass for version 0.4.0 in [PR 15](https://github.com/Andreasniss/ai-sdlc-skill/pull/15), whose checks identify the exact tested revision. A [partial matched behavior pilot](skills/ai-sdlc-skill/evals/feedback-pilot.md) covers three cases without demonstrating an advantage over version 0.3.0. The remaining ten behavior cases were not rerun. 7DayFocus and Runbook Relay have earlier merged repository integrations with application checks and pinned source manifests; those integrations have not been rerun for this revision. Installation instructions follow the linked product and installer documentation. File copying, installer discovery, and helper tests are separate from interactive compatibility or better decisions across assistants. Record the actual host and observed behavior when you try the first task.
...
The [20 September review-loop evaluation](skills/ai-sdlc-skill/evals/review-loop-evaluation.md) records the current 15-case behavior set on one Codex host. It used explicit file loading, not a native installation test; its results do not establish cross-host installation or interactive compatibility.
