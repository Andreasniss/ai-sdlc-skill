...
The [20 September review-loop evaluation](skills/ai-sdlc-skill/evals/review-loop-evaluation.md) records the 15-case behavior set as it stood at 0.4.1, on one Codex host. The set now has 17 cases, so that record no longer validates against it. It also used explicit file loading, not a native installation test; its results do not establish cross-host installation or interactive compatibility.
